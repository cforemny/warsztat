...
public class LuzyckaController {
    ...
    @PostMapping("Luzycka/dodajDate")
    public String addNewColumn(@ModelAttribute CourseDate courseDate, Model model) {
        studentListCreator.addNewDate("datyluzycka", courseDate.getCurrentDate());
        prepareSiteObjects(model);
        return "lokalizacje/Luzycka";
    }

    @PostMapping("Luzycka/platnosci")
    public String addPayment(@ModelAttribute Payment payment, Model model) {
        studentListCreator.addNewPayment("platnosciluzycka", payment.getPaymentValue(), payment.getStudentId(), payment.getPaymentDate(), payment.getPaymentType());
        prepareSiteObjects(model);
        return "lokalizacje/Luzycka";
    }
